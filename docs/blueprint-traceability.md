...
| 7 | quote evidence + fail-closed payment capability boundary | Quotes create no PSP intent; `PAYMENTS_MODE` remains `OFF` until verified provider evidence can drive a supplied state map |
...
- Capability tests prove this adapter-less build rejects payment `LIVE` and `SANDBOX` at startup
  and publishes only `OFF` in readiness output.
...
- Real-PostgreSQL tests cover rotation, booking, ledger reconciliation, and ledger permissions.
  Payment intents, webhook signature verification, and idempotency remain release gates pending a
  supplied PSP contract and adapter.
